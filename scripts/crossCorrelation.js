var countArray = [];
for (var i = 0, t = chirpX.length*2-1; i < t; i++) {
    countArray.push(i + 1);
}
...
for (delay = -maxdelay + 1; delay < maxdelay; delay++) {
    ...
    for (i = 0, n = maxdelay; i < n; i++) {
        ...
        if (j < 0 || j >= n)
            ...
            sxy += chirpY[i] * chirpY[j];
    }
    xcorr.push(sxy);
}
corPoint = Math.max.apply(Math, xcorr)
var Index = xcorr.indexOf(Math.max.apply(Math, xcorr));
//document.write("Delay Array; ", xcorr, "<br>", "Length of array: ", xcorr.length, " Correlation Point: ", corPoint, " Index of Correltion Point: ", Index, "<br>");
document.write("<br>", "Length of array: ", xcorr.length, " Correlation Point: ", corPoint, " Index of Correltion Point: ", Index, "<br>");
